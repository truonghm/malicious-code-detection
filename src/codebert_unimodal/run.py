...
class IterableTextDataset(IterableDataset):
    ...
    def load_data(self):
        with open(self.file_path) as f:
            for line in f:
                js = json.loads(line.strip())
                yield convert_examples_to_features(js, self.tokenizer, self.args)

    def __iter__(self):
        data = self.load_data()
        return torch.tensor(data.input_ids), torch.tensor(data.label)
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.num_train_epochs):
        ...
        if results["eval_acc"] > best_acc:
            ...
            checkpoint_prefix = "checkpoint-best-acc-codebert"
...
def evaluate(args, model, tokenizer):
    ...
    # eval_dataset = IterableTextDataset(tokenizer, args, args.eval_data_file)
    ...
    eval_dataloader = DataLoader(
        eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, num_workers=4, pin_memory=True
    )
...
def test(args, model, tokenizer):
    ...
    # eval_dataset = IterableTextDataset(tokenizer, args, args.test_data_file)
    eval_sampler = SequentialSampler(eval_dataset)
...
def main():
    ...
    parser.add_argument(
        "--model_name_or_path", default=None, type=str, help="The model checkpoint for weights initialization."
    )
    parser.add_argument(
        "--tokenizer_name",
        default="",
    ...
    parser.add_argument("--num_train_epochs", type=int, default=42, help="num_train_epochs")
    ...
    if args.do_train:
        ...
        # train_dataset = IterableTextDataset(tokenizer, args, args.train_data_file)
